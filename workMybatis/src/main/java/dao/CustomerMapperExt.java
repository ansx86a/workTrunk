...
//	List<HashMap> 物理分頁(PageBounds pageBound, HashMap c);
    ...
    String NARIVE_SQL = "${sql}";
    @Select({NARIVE_SQL})
    List<HashMap> 當有要用動態sql要用native_sql時候使用(TableParameter param);

    class TableParameter {
        private final static String SQL = "select * from {0}.{1} limit 100";
        private final String schema;
        private final String table;

        public TableParameter(String schema, String table) {
            schema.matches("\\w+");
            table.matches("\\w+");
            this.schema = schema;
            this.table = table;
